...
; Updated 20241105: Speed updates from Peter Jauernig
...
;---------------------------------------------------------------
;Peter Jauernig is credited with the following speedups:
; baseline              shuttle  87554 text 45039 robotron 319882 ...
; opt. for shorter cnt: shuttle  87226 text 44993 robotron 319321 +++
; unroll 2nd leng. cnt: shuttle  87203 text 44993 robotron 319308 +*+
...
lz4_decompress_ PROC
        ...
        les     di,[_outb]       ;load target buffer
        push    di              ;save original starting offset (in case != 0)
        lds     si,[_inb]        ;load source buffer
...
starttoken:
        ...
        xlat    cs:[bx]            ;unpack upper 4 bits, faster than SHR reg,cl
...
buildlcount:                    ;build full literal length count
        ...
        jne     @@doliteralcopy ;if not (more common case), done counting
...
buildmcount:                    ;build full match length count - AX is 0
        ...
        add     cx,ax           ;increase count
        cmp     al,0FFh         ;more count bytes to read?
        jne     @@domatchcopy   ;if not (more common case), done counting
